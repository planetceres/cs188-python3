...
class Gridworld(mdp.MarkovDecisionProcess):
    ...
    def getStartState(self):
        for x in range(self.grid.width):
            for y in range(self.grid.height):
                if self.grid[x][y] == 'S':
                    return (x, y)
        raise EnvironmentError('Grid has no start state')
    ...
    def getTransitionStatesAndProbs(self, state, action):
        ...
        if action not in self.getPossibleActions(state):
            raise EnvironmentError("Illegal action!")
...
class GridworldEnvironment(environment.Environment):
    ...
    def getRandomNextState(self, state, action, randObj=None):
        ...
        for nextState, prob in successors:
            sum += prob
            if sum > 1.0:
                raise EnvironmentError('Total transition probability more than one; sample failure.')
            if rand < sum:
                reward = self.gridWorld.getReward(state, action, nextState)
                return (nextState, reward)
        raise EnvironmentError('Total transition probability less than one; sample failure.')
...
def runEpisode(agent, environment, discount, decision, display, message, pause, episode):
    ...
    while True:
        ...
        if action == None:
            raise EnvironmentError('Error: Agent returned None action')
